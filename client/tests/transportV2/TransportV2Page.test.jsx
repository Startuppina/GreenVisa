...
describe("TransportV2Page", () => {
  ...
  it("resumes autosave after the next edit following a 409 response", async () => {
    ...
    const ecoDriveSelect = screen.getByRole("combobox", {
      name: /Eco-drive training coverage/i,
    });
    const classATiresSelect = screen.getByRole("combobox", {
      name: /Class A tire adoption/i,
    });
  ...
  it("applies OCR output into the normal vehicle workflow", async () => {
    ...
    await user.upload(
      fileInput,
      new File(["test-pdf"], "vehicle.pdf", { type: "application/pdf" }),
    );
    ...
    await waitFor(() => {
      const co2Input = screen.getByRole("spinbutton", {
        name: /CO₂ emissions \(g\/km\)/i,
      });
    ...
    it("shows confirmation dialog when Submit is clicked and cancelling does not submit", async () => {
      ...
      server.use(
        ...
        createTransportV2SubmitHandler(() => {
          submitCalled = true;
          return HttpResponse.json({
            transport_v2: makeSubmittedDraftFixture(),
          });
        }),
      );
      ...
      expect(
        screen.getByText(/Conferma invio questionario/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/non sarà più possibile modificare/i),
      ).toBeInTheDocument();

      await user.click(screen.getByRole("button", { name: /Annulla/i }));

      expect(
        screen.queryByText(/Conferma invio questionario/i),
  ...
  describe("post-submit read-only state", () => {
    ...
    it("hides the OCR upload panel entirely", async () => {
      ...
      expect(
        screen.queryByLabelText(/upload one or more documents/i),
      ).not.toBeInTheDocument();
    });
