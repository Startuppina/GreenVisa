function JsonBlock({ title, value }) {
  const hasValue = value && Object.keys(value).length > 0;

  return (
    <div className="rounded-xl bg-slate-50 p-4">
      <div className="text-sm font-medium text-slate-900">{title}</div>
      {hasValue ? (
        <pre className="mt-3 overflow-x-auto whitespace-pre-wrap text-xs text-slate-700">
          {JSON.stringify(value, null, 2)}
        </pre>
      ) : (
        <div className="mt-2 text-sm text-slate-500">Nessun dato dal server ancora disponibile.</div>
      )}
    </div>
  );
}

export default function TransportResultsPanel({ derived, results }) {
  const score = results?.score;
  const co2 = results?.co2;
  const co2TonsPerYear =
    co2?.total_tons_per_year === null || co2?.total_tons_per_year === undefined
      ? null
      : Number(co2.total_tons_per_year);
  const co2TonsPerYearDisplay =
    co2TonsPerYear === null || Number.isNaN(co2TonsPerYear)
      ? 'n/d'
      : co2TonsPerYear.toFixed(2);

  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-5">
        <h2 className="text-xl font-semibold text-slate-900">Risultati dal server</h2>
        <p className="mt-1 text-sm text-slate-600">
          Valori solo in lettura, restituiti dal server dopo l&apos;invio del questionario.
        </p>
      </div>

      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl bg-emerald-50 p-4">
          <div className="text-sm font-medium text-emerald-900">Punteggio totale</div>
          <div className="mt-2 text-3xl font-semibold text-emerald-700">
            {score?.total_score ?? 'n/d'}
          </div>
        </div>
        <div className="rounded-xl bg-sky-50 p-4">
          <div className="text-sm font-medium text-sky-900">CO₂ tonnellate/anno</div>
          <div className="mt-2 text-3xl font-semibold text-sky-700">
            {co2TonsPerYearDisplay}
          </div>
        </div>
        <div className="rounded-xl bg-slate-50 p-4">
          <div className="text-sm font-medium text-slate-900">Versione calcolatore</div>
          <div className="mt-2 text-2xl font-semibold text-slate-700">
            {results?.calculator_version || 'n/d'}
          </div>
        </div>
      </div>

      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        <JsonBlock title="Dati derivati" value={derived} />
        <JsonBlock title="Risultati (dettaglio)" value={results} />
      </div>
    </section>
  );
}
