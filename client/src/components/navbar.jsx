...
function Navbar() {
    ...
    return (
        <nav className="flex space-between center p-0 bg-transparent h-[100px] md:h-[150px]">
            <Link to="/"><img src="/img/logo.png" alt="Green Visa Logo" className="w-[100px] h-[100px] md:w-[150px] md:h-[150px] absolute top-0 left-4 z-10 cursor-pointer" /></Link>
            <div className={showMenu ? "hidden" : "absolute top-[30px] right-[15px]"}>
                <div>
                    <ul className="list-none flex gap-[20px] font-[Arial] text-[1.2em] font-bold m-0">
                        ...
                        <li className="cursor-pointer hover:text-[#2d7044]"><img src="/img/user-icon.png" alt="user" style={{ width: "30px", height: "30px" }} onClick={handleUserIconClick} /></li>
                    </ul>
                    <img src="/img/menu.png" alt="menu" className={showMenu ? "block w-[30px] h-[30px] cursor-pointer absolute right-0 m-0" : "hidden"} />
                </div>
                <img src="/img/search-icon.png" alt="search" className="w-[30px] h-[30px] cursor-pointer absolute right-0 mt-[15px]" onClick={() => showFinder(!finder)} />
            ...
            <div className={showMenu ? "absolute top-0 left-0 right-0 bottom-0" : "hidden"}>
                <img src="/img/menu.png" alt="menu" className="w-[35px] h-[35px] cursor-pointer absolute right-4 top-4" onClick={() => setOpenDrawer(!openDrawer)} />
            </div>
            ...
            <div className={"fixed top-0 right-0 h-full w-64 transform transition-transform duration-300 bg-white p-4 z-30 " + (openDrawer ? "translate-x-0" : "translate-x-full")}>
                <img src="/img/close.png" alt="close" className="w-[40px] h-[40px] cursor-pointer absolute right-4 top-4" onClick={() => setOpenDrawer(!openDrawer)} />
