...
function VehicleCard({ vehicle, index, disabled, onUpdateVehicle, onRemoveVehicle }) {
  ...
  const updateVehicleField = (fieldKey, value) => {
    updateVehicle((currentVehicle) => ({
      ...currentVehicle,
      fields: {
        ...(currentVehicle.fields || {}),
        [fieldKey]: value,
      },
    }));
  };

  const updateVehicleFieldAndCleanup = (fieldKey, value) => {
    updateVehicle((currentVehicle) => {
      const nextSources = { ...(currentVehicle.field_sources || {}) };
      const nextWarnings = { ...(currentVehicle.field_warnings || {}) };
      delete nextSources[fieldKey];
      delete nextWarnings[fieldKey];
    ...
    <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <NumberField
          ...
          disabled={disabled}
          onChange={(value) => updateVehicleFieldAndCleanup('registration_year', value)}
        />
        ...
        <SelectField
          ...
          disabled={disabled}
          onChange={(value) => updateVehicleFieldAndCleanup('euro_class', value || null)}
        />
        ...
        <SelectField
          ...
          disabled={disabled}
          onChange={(value) => updateVehicleFieldAndCleanup('fuel_type', value || null)}
        />
        ...
        <SelectField
          ...
          disabled={disabled}
          onChange={(value) =>
            updateVehicleFieldAndCleanup('wltp_homologation', parseBooleanSelectValue(value))
          }
        />
        ...
        <SelectField
          label="Profilo passeggeri"
          value={vehicle?.fields?.occupancy_profile_code?.toString?.() || ''}
          options={PROFILE_CODE_OPTIONS}
          disabled={disabled}
          onChange={(value) => updateVehicleField('occupancy_profile_code', parseIntegerSelectValue(value))}
        />
        ...
        <SelectField
          ...
          options={PROFILE_CODE_OPTIONS}
          disabled={disabled}
          onChange={(value) => updateVehicleField('load_profile_code', parseIntegerSelectValue(value))}
        />

        <label className="flex flex-col gap-2 text-sm text-slate-700">
          <span className="font-medium text-slate-800">Data ultima revisione</span>
          <input
            type="date"
            className={inputClassName}
