...
export default function TransportV2GlobalQuestionsCard({
  questionnaireFlags,
  disabled,
  onChangeFlag,
}) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-5">
        <h2 className="text-lg font-semibold text-slate-900">Informazioni generali</h2>
        <p className="mt-1 text-sm text-slate-600">
          Queste risposte valgono per tutto il parco mezzi del draft condiviso.
        </p>
      </div>
        ...
        {TRANSPORT_V2_FLAG_FIELDS.map((field) => (
          <label key={field.key} className="flex flex-col gap-2 text-sm text-slate-700">
            ...
            <select
              ...
              disabled={disabled}
              onChange={(event) =>
                onChangeFlag(field.key, parseBooleanSelectValue(event.target.value))
              }
            >
